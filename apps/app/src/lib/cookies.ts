...
const ONBOARDED_COOKIE_NAME = "is_onboarded";
...
/**
 * Устанавливает cookie статуса онбординга
 */
export function setOnboardedCookie(isOnboarded: boolean): void {
  if (typeof document === "undefined") return;
  const isSecure = window.location.protocol === "https:";
  const cookieString = `${ONBOARDED_COOKIE_NAME}=${isOnboarded}; path=/; max-age=${COOKIE_MAX_AGE}; SameSite=Lax${isSecure ? "; Secure" : ""}`;
  // biome-ignore lint/suspicious/noDocumentCookie: Cookie Store API has limited browser support
  document.cookie = cookieString;
}

/**
 * Очищает cookie статуса онбординга
 */
export function clearOnboardedCookie(): void {
  if (typeof document === "undefined") return;
  // biome-ignore lint/suspicious/noDocumentCookie: Cookie Store API has limited browser support
  document.cookie = `${ONBOARDED_COOKIE_NAME}=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT`;
}
