...
export function useUserFormValidation() {
  ...
  const validateForm = useCallback((form: EditUserForm): FormValidationError[] => {
    ...
    if (form.email && form.email.trim()) {
      if (!validateEmail(form.email)) {
        newErrors.push({ field: "email", message: "Укажите корректный email адрес." });
      }
    }
    ...
    if (form.filter_min_duration < 0) {
      newErrors.push({ 
        ...
        message: "Минимальная длительность звонка не может быть отрицательной." 
      });
    }
