...
export default function ReportSettingsPanel({ user }: { user: User }) {
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      filterMinDuration: String(Number(u.filter_min_duration) || 0),
      filterMinReplicas: String(Number(u.filter_min_replicas) || 0),
      kpiBaseSalary: String(Number(u.kpi_base_salary) || 0),
      kpiTargetBonus: String(Number(u.kpi_target_bonus) || 0),
