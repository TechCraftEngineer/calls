...
interface CallListProps {
    ...
    onRecommendationsGenerated?: (callId: number, recommendations: string[]) => void;
}

type SortKey = 'type' | 'number' | 'manager' | 'status' | 'date' | 'score' | 'summary' | 'duration';
...
const COLUMNS: ColumnConfig[] = [
    { key: 'type', label: 'Тип', tooltip: 'Направление звонка: Входящий или Исходящий', sortKey: 'type' },
    { key: 'number', label: 'Номер клиента', tooltip: 'Телефонный номер клиента и внутренний номер сотрудника', sortKey: 'number' },
    { key: 'manager', label: 'Сотрудник', tooltip: 'Имя сотрудника, участвовавшего в разговоре', sortKey: 'manager' },
    { key: 'status', label: 'Статус', tooltip: 'Результат звонка: Принят или Пропущен', sortKey: 'status' },
    { key: 'date', label: 'Дата', tooltip: 'Дата и время начала звонка', sortKey: 'date' },
    { key: 'score', label: 'Ценность', tooltip: 'Оценка качества звонка ИИ (от 1 до 5 звезд)', sortKey: 'score' },
    { key: 'summary', label: 'Вывод', tooltip: 'Краткое резюме разговора, составленное ИИ', sortKey: 'summary' },
    { key: 'record', label: 'Запись', tooltip: 'Возможность прослушать аудиозапись разговора' },
    { key: 'duration', label: 'Длительность', tooltip: 'Общая продолжительность разговора', sortKey: 'duration' },
];
...
export default function CallList({ calls, onPlay, user, onCallDeleted, onRecommendationsGenerated }: CallListProps) {
    ...
    const [recommendationsCallId, setRecommendationsCallId] = useState<number | null>(null);
    ...
    const handleGenerateRecommendations = async (callId: number, existingRecommendations?: string[]) => {
    ...
    return (
        <div style={{ position: 'relative' }}>
            ...
            <div style={{
                ...
                {showColumnToggle && (
                    <div style={{
                        ...
                        <div style={{ fontSize: '12px', fontWeight: 700, marginBottom: '8px', color: '#999', textTransform: 'uppercase' }}>Видимость колонок</div>
            ...
            <table className="op-table">
                <thead>
                    <tr>
                        {orderedColumns.map(col => visibleColumns.includes(col.key) && (
                            <th
                                key={col.key}
                                draggable={true}
                                ...
                                style={{
                                    ...
                                    userSelect: 'none',
                                    position: 'relative',
                                    backgroundColor: dragOverColumn === col.key ? '#f0f8ff' : draggedColumn === col.key ? '#f5f5f5' : 'transparent',
                            ...
                            >
                                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                    ...
                                    <div className="op-tooltip" style={{ display: 'inline-flex' }}>
                                        <span className="info-icon" style={{ margin: 0 }}>i</span>
                                        <div className="tooltip-content" style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>
                                            {col.tooltip}
                                        </div>
                                    </div>
                ...
                <tbody>
                    {sortedCalls.map((item) => {
                        ...
                        const isMissed = call.duration_seconds === 0 && (call.direction === 'Входящий' || call.direction === 'incoming');

                        const directionLabel = (call.direction === 'incoming' || call.direction === 'Входящий') ? 'ВХОДЯЩИЙ' : 'ИСХОДЯЩИЙ';
                        ...
                        const renderCell = (colKey: string) => {
                            ...
                            switch (colKey) {
                                    ...
                                    return (
                                        <td key={colKey}>
                                            {call.customer_name ? (
                                                <>
                                                    {isMobileDevice() ? (
                                                        <Link href={`/calls/${call.id}`} className="call-link" style={{ fontSize: '15px', fontWeight: 700, display: 'block', marginBottom: '4px' }}>
                                                            {call.customer_name}
                                                        </Link>
                                                    ) : (
                                                        <button
                                                ...
                                                <>
                                                    {isMobileDevice() ? (
                                                        <Link href={`/calls/${call.id}`} className="call-link">
                                                            {call.number}
                                                        </Link>
                                                    ) : (
                                                        <button
                                                    ...
                                                    <div style={{ fontSize: '11px', color: '#999', marginTop: '2px' }}>{call.internal_number || ''}</div>
                                                </>
                                            )}
                                        </td>
                                    );
                                case 'manager':
                                    return (
                                        <td key={colKey}>
                                            <span style={{ color: '#555', fontWeight: 500 }}>{call.manager_name || call.operator_name || '—'}</span>
                                        </td>
                                    );
                                case 'status':
                                    return (
                                        ...
                                        <td key={colKey}>
                                            {evaluation?.value_score ? (
                                                <div className="op-tooltip">
                                                    <div style={{ color: '#FFD600', fontSize: '16px', letterSpacing: '2px' }}>
                                                        {'★'.repeat(evaluation.value_score)}{'☆'.repeat(5 - evaluation.value_score)}
                                                    </div>
                                ...
                                case 'summary':
                                    return (
                                        <td key={colKey}>
                                            {transcript?.summary ? (
                                                <div className="op-tooltip" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                    <span style={{
                                                        maxWidth: '220px',
                                                        whiteSpace: 'nowrap',
                                                        overflow: 'hidden',
                                                    ...
                                                    <>
                                                        <button
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                e.stopPropagation();
                                                                handleGenerateRecommendations(call.id, evaluation?.manager_recommendations);
                                                            }}
                                                            ...
                                                            style={{
                                                                background: 'none',
                                                                border: 'none',
                                                                cursor: isLoadingRecommendations && recommendationsCallId === call.id ? 'wait' : 'pointer',
                                                                padding: '4px',
                                                                display: 'flex',
                                                                alignItems: 'center',
                                                                justifyContent: 'center',
                                                                ...
                                                                opacity: isLoadingRecommendations && recommendationsCallId === call.id ? 0.6 : 1
                                                            }}
                                                            onMouseEnter={(e) => {
                                                                if (!isLoadingRecommendations || recommendationsCallId !== call.id) {
                                                        ...
                                                        >
                                                            {isLoadingRecommendations && recommendationsCallId === call.id ? (
                                                                ...
                                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ color: '#F7931E' }}>
                                                                    <path d="M9 21h6M12 3a6 6 0 0 1 6 6c0 2.22-1.21 4.15-3 5.19V17H9v-2.81c-1.79-1.04-3-2.97-3-5.19a6 6 0 0 1 6-6z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
                                                                </svg>
                                                            )}
                                                        </button>
                                                        <button
                                                            ...
                                                            onClick={(e) => {
                                                                ...
                                                                if (onPlay && call.filename) {
                                                                    ...
                                                                    window.open(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/records/${call.filename}`, '_blank');
                                                                }
                                                            }}
                                                            title="Прослушать запись"
                                                        >
                                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style={{ marginLeft: '2px' }}>
                        ...
                        const dateKey = call.timestamp ? new Date(call.timestamp).toISOString().slice(0, 10) : '';
