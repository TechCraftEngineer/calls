...
export default function UsersPage() {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setAddError('');
    if (!addForm.username.trim() || !addForm.password.trim() || !addForm.first_name.trim()) {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.users.update({
        ...
        filter_exclude_answering_machine: editForm.filter_exclude_answering_machine,
    ...
    <div className="app-container">
      ...
      <main className="main-content">
        <header className="page-header" style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p className="page-subtitle" style={{ marginTop: '8px', fontSize: '14px', color: '#999' }}>
              ...
              {loading ? (
                ...
                users.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td style={{ fontWeight: 600, color: '#333' }}>{u.username}</td>
                    <td style={{ color: '#555' }}>{u.first_name || '—'}</td>
                    <td style={{ color: '#555' }}>{u.last_name || '—'}</td>
                    <td style={{ color: '#555', fontWeight: 500 }}>{u.internal_numbers || '—'}</td>
                    <td style={{ color: '#555', fontWeight: 500 }}>{u.mobile_numbers || '—'}</td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', alignItems: 'center' }}>
                        ...
                        <div style={{ width: '80px', display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
                          {currentUser?.id === u.id ? (
                            <span style={{ fontSize: '11px', color: '#999', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                              ЭТО ВЫ
                            </span>
                          ) : (
                            <button
                ...
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: '40px', color: '#999' }}>Нет данных</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showAddModal && (
        <div style={modalOverlayStyle} onClick={() => setShowAddModal(false)}>
          <div style={modalBoxStyle} onClick={(e) => e.stopPropagation()}>
            <h2 style={{ margin: '0 0 20px', fontSize: '18px', fontWeight: 700 }}>Добавить пользователя</h2>
            <form onSubmit={handleAddSubmit}>
              {addError && <p style={{ color: '#c00', marginBottom: '12px', fontSize: '14px' }}>{addError}</p>}
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Логин *</label>
                <input
                  type="text"
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Пароль *</label>
                <input
                  type="password"
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Имя *</label>
                <input
                  type="text"
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Фамилия</label>
                <input
                  type="text"
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Внутренние номера</label>
                <input
                  type="text"
                  value={addForm.internal_numbers}
                  onChange={(e) => setAddForm((f) => ({ ...f, internal_numbers: e.target.value }))}
                  style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                  placeholder="101, 102 или admin, ovchinnikov_nikita (МегаФон)"
                />
              </div>
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Мобильные номера</label>
                <input
                  type="text"
                  value={addForm.mobile_numbers}
                  onChange={(e) => setAddForm((f) => ({ ...f, mobile_numbers: e.target.value }))}
                  style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
              ...
              <div style={{ marginBottom: '16px', padding: '16px', background: '#f5f7fa', borderRadius: '8px' }}>
                <h3 style={{ margin: '0 0 12px', fontSize: '14px', fontWeight: 700 }}>Telegram Отчеты</h3>
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setAddForm((f) => ({ ...f, telegram_chat_id: e.target.value }))}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setAddForm((f) => ({ ...f, telegram_daily_report: e.target.checked }))}
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setAddForm((f) => ({ ...f, telegram_manager_report: e.target.checked }))}
                    />
              ...
              <div style={{ marginBottom: '16px', padding: '16px', background: '#f5f7fa', borderRadius: '8px' }}>
                ...
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>MAX Chat ID</label>
                  <input
                    type="text"
                    value={addForm.max_chat_id}
                    onChange={(e) => setAddForm((f) => ({ ...f, max_chat_id: e.target.value }))}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setAddForm((f) => ({ ...f, max_daily_report: e.target.checked }))}
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setAddForm((f) => ({ ...f, max_manager_report: e.target.checked }))}
                    />
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                <button type="button" onClick={() => setShowAddModal(false)} style={{ padding: '8px 16px', border: '1px solid #ddd', borderRadius: '6px', background: 'white', cursor: 'pointer' }}>Отмена</button>
                <button type="submit" disabled={addSubmitting} style={{ padding: '8px 16px', border: 'none', borderRadius: '6px', background: 'linear-gradient(135deg, #FF6B35 0%, #F7931E 100%)', color: 'white', fontWeight: 600, cursor: addSubmitting ? 'not-allowed' : 'pointer' }}>{addSubmitting ? 'Сохранение…' : 'Добавить'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showEditModal && editUser && (
        <div style={modalOverlayStyle} onClick={() => setShowEditModal(false)}>
          <div style={modalBoxStyle} onClick={(e) => e.stopPropagation()}>
            <h2 style={{ margin: '0 0 20px', fontSize: '18px', fontWeight: 700 }}>Редактировать пользователя</h2>
            <p style={{ margin: '0 0 16px', fontSize: '13px', color: '#666' }}>Логин: {editUser.username}</p>
            <form onSubmit={handleEditSubmit}>
              {editError && <p style={{ color: '#c00', marginBottom: '12px', fontSize: '14px' }}>{editError}</p>}
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Имя *</label>
                <input
                  type="text"
              ...
              <div style={{ marginBottom: '12px' }}>
                ...
                <input
                  ...
                  onChange={(e) => setEditForm((f) => ({ ...f, internal_numbers: e.target.value }))}
                  style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                  placeholder="101, 102 или admin, ovchinnikov_nikita (МегаФон)"
                />
              </div>
              <div style={{ marginBottom: '16px' }}>
                ...
                <input
                  ...
                  onChange={(e) => setEditForm((f) => ({ ...f, mobile_numbers: e.target.value }))}
                  style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                  placeholder="79XXXXXXXXX, можно несколько через запятую"
                />
              </div>
              ...
              <div style={{ marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input
                    ...
                    style={{ flex: 1, padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                    placeholder="ID чата пользователя"
                  />
                </div>
                <div style={{ marginTop: '8px' }}>
                  {editUser.telegram_chat_id ? (
                    <button
                      ...
                      style={{
                        fontSize: '13px', color: '#FF5252', background: 'none', border: '1px solid #FF5252', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer'
                      }}
                    >
                      Отвязать Telegram
                    </button>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      <button
                        ...
                        style={{
                          fontSize: '13px', color: '#0088cc', background: 'none', border: '1px solid #0088cc', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px'
                        }}
                      >
                      ...
                      <button
                        ...
                        style={{
                          fontSize: '13px', color: '#666', background: 'none', border: '1px solid #ddd', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer'
                        }}
                      >
              ...
              <div style={{ marginBottom: '16px', padding: '16px', background: '#f5f7fa', borderRadius: '8px' }}>
                ...
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm((f) => ({ ...f, max_chat_id: e.target.value }))}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                    placeholder="ID чата MAX"
                  />
                </div>
                ...
                <div style={{ marginBottom: '12px' }}>
                  {editUser.max_chat_id ? (
                    ...
                    <button
                      ...
                      style={{
                        fontSize: '13px', color: '#6f42c1', background: 'none', border: '1px solid #6f42c1', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px'
                      }}
                    >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, max_daily_report: e.target.checked }))}
                    />
                    Получать свои ежедневные отчеты (MAX)
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, max_manager_report: e.target.checked }))}
                    />
                    Получать отчеты по всем менеджерам (MAX)
                  </label>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, telegram_daily_report: e.target.checked }))}
                    />
                    Ежедневный отчет
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, telegram_weekly_report: e.target.checked }))}
                    />
                    Еженедельный отчет
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, telegram_monthly_report: e.target.checked }))}
                    />
                    Ежемесячный отчет
                  </label>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, email_daily_report: e.target.checked }))}
                    />
                    Ежедневный отчет
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, email_weekly_report: e.target.checked }))}
                    />
                    Еженедельный отчет
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, email_monthly_report: e.target.checked }))}
                    />
                    Ежемесячный отчет
                  </label>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, report_detailed: e.target.checked }))}
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, report_include_call_summaries: e.target.checked }))}
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, report_include_avg_value: e.target.checked }))}
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setEditForm((f) => ({ ...f, report_include_avg_rating: e.target.checked }))}
                    />
              ...
              <div style={{ marginBottom: '16px', padding: '16px', background: '#f5f7fa', borderRadius: '8px' }}>
                ...
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Базовый оклад (₽)</label>
                  <input
                    type="number"
                    value={editForm.kpi_base_salary}
                    onChange={(e) => setEditForm((f) => ({ ...f, kpi_base_salary: parseFloat(e.target.value) || 0 }))}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                  />
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Целевой бонус (₽)</label>
                  <input
                    type="number"
                    value={editForm.kpi_target_bonus}
                    onChange={(e) => setEditForm((f) => ({ ...f, kpi_target_bonus: parseFloat(e.target.value) || 0 }))}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                  />
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Целевое время разговоров в месяц (мин)</label>
                  <input
                    type="number"
                    value={editForm.kpi_target_talk_time_minutes}
                    onChange={(e) => setEditForm((f) => ({ ...f, kpi_target_talk_time_minutes: parseFloat(e.target.value) || 0 }))}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                  />
                </div>
              </div>
              ...
              <div style={{ marginBottom: '16px', padding: '16px', background: '#f5f7fa', borderRadius: '8px' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer', marginBottom: '12px' }}>
                  <input
                    ...
                    onChange={(e) => setEditForm((f) => ({ ...f, filter_exclude_answering_machine: e.target.checked }))}
                  />
                  Автоответчики
                </label>
                <div style={{ marginBottom: '8px' }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm((f) => ({ ...f, filter_min_duration: parseInt(e.target.value, 10) || 0 }))}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm((f) => ({ ...f, filter_min_replicas: parseInt(e.target.value, 10) || 0 }))}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '6px', boxSizing: 'border-box' }}
                    placeholder="0 — не исключать"
                  />
                </div>
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                <button type="button" onClick={() => setShowEditModal(false)} style={{ padding: '8px 16px', border: '1px solid #ddd', borderRadius: '6px', background: 'white', cursor: 'pointer' }}>Отмена</button>
                <button type="submit" disabled={editSubmitting} style={{ padding: '8px 16px', border: 'none', borderRadius: '6px', background: 'linear-gradient(135deg, #FF6B35 0%, #F7931E 100%)', color: 'white', fontWeight: 600, cursor: editSubmitting ? 'not-allowed' : 'pointer' }}>{editSubmitting ? 'Сохранение…' : 'Сохранить'}</button>
              </div>
            </form>
          </div>
        </div >
...

      {
        showPasswordModal && passwordUser && (
          <div style={modalOverlayStyle} onClick={() => setShowPasswordModal(false)}>
            <div style={modalBoxStyle} onClick={(e) => e.stopPropagation()}>
              <h2 style={{ margin: '0 0 20px', fontSize: '18px', fontWeight: 700 }}>Сменить пароль</h2>
              <p style={{ margin: '0 0 16px', fontSize: '13px', color: '#666' }}>Пользователь: {passwordUser.username}</p>
              <form onSubmit={handlePasswordSubmit}>
                {passwordError && <p style={{ color: '#c00', marginBottom: '12px', fontSize: '14px' }}>{passwordError}</p>}
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Новый пароль *</label>
                  <input
                    type="password"
                ...
                <div style={{ marginBottom: '16px' }}>
                  <label style={{ display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600 }}>Подтверждение пароля *</label>
                  <input
                    type="password"
                ...
                <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                  <button type="button" onClick={() => setShowPasswordModal(false)} style={{ padding: '8px 16px', border: '1px solid #ddd', borderRadius: '6px', background: 'white', cursor: 'pointer' }}>Отмена</button>
                  <button type="submit" disabled={passwordSubmitting} style={{ padding: '8px 16px', border: 'none', borderRadius: '6px', background: 'linear-gradient(135deg, #FF6B35 0%, #F7931E 100%)', color: 'white', fontWeight: 600, cursor: passwordSubmitting ? 'not-allowed' : 'pointer' }}>{passwordSubmitting ? 'Сохранение…' : 'Сменить пароль'}</button>
                </div>
              </form>
            </div>
          </div>
