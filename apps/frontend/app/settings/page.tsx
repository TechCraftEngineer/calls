    ...
    <div className="app-container">
      ...
      <main className="main-content">
        ...
        <section className="card" style={{ marginBottom: '24px' }}>
          <div className="section-title" style={{ marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '18px' }}>📊</span> Отчёты в Telegram
          </div>
          <p style={{ marginBottom: '16px', fontSize: '14px', color: '#555' }}>
            Подписки на ежедневный/еженедельный/ежемесячный отчёт и опция «не отправлять в выходные» настраиваются на странице{' '}
            <Link href="/statistics?tab=settings" style={{ color: '#FF6B35', fontWeight: 600 }}>Статистика</Link> → вкладка «Настройки отчетов».
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setSendTestMessage(typeof d === 'string' ? d : 'Не удалось отправить. Укажите Telegram Chat ID в Настройках отчётов.');
                } finally {
              ...
              style={{
                ...
                border: 'none',
                borderRadius: '8px',
                background: sendTestLoading ? '#ccc' : 'linear-gradient(135deg, #4CAF50 0%, #388E3C 100%)',
                color: 'white',
                fontWeight: 600,
            ...
            >
              {sendTestLoading ? 'Отправка...' : 'Отправить тестовый отчёт в Telegram'}
            </button>
            <Link
            ...
            {sendTestMessage && (
              <span style={{ color: sendTestMessage.includes('отправлен') ? '#4CAF50' : '#FF5252', fontSize: '14px' }}>
                {sendTestMessage}
              </span>
            )}
          </div>
        ...
        <section className="card" style={{ marginBottom: '24px' }}>
          <div className="section-title" style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="filter-item">
              <label className="filter-label">ПОРОГ ЦЕННОСТИ ДЛЯ ОЦЕНКИ КАЧЕСТВА (0-5)</label>
              <input
                type="number"
                min="0"
                max="5"
            ...
            <div style={{ fontSize: '13px', color: '#666', fontStyle: 'italic', marginTop: '-12px' }}>
              * Оценка качества будет выполняться только для звонков с ценностью не ниже указанного порога.
            </div>

            <div className="filter-item">
              <label className="filter-label" style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}>
                <input
                  type="checkbox"
              ...
              <div style={{ fontSize: '13px', color: '#666', fontStyle: 'italic', marginTop: '4px', marginLeft: '30px' }}>
                * Если включено, ИИ будет предлагать вопросы, которые менеджер мог бы задать клиенту.
              </div>
            </div>
          </div>
        </section>
        ...
        {Object.entries(promptKeys).map(([key, title]) => {
          ...
          const prompt = prompts[key] || { key, value: '', description: '', updated_at: undefined };
          const isTokenOnly = tokenOnlyKeys.includes(key);
          const variables = extractVariables(prompt.value || '');

          return (
            <section key={key} className="card" style={{ marginBottom: '24px' }}>
                ...
                <>
                  {variables.length > 0 && (
                    <div style={{ marginBottom: '16px' }}>
                      <div className="filter-label" style={{ marginBottom: '8px' }}>Доступные переменные:</div>
                  ...
                  <div className="filter-item" style={{ marginBottom: '16px' }}>
                    ...
                    <input
                      ...
                      onChange={e => {
                        const updated = { ...prompts };
                        updated[key] = { ...prompt, description: e.target.value };
                        setPrompts(updated);
                      }}
        ...
        {currentUser?.username === 'admin@mango' && (
          <section className="card" style={{ marginBottom: '24px' }}>
            ...
            <button
              ...
              style={{
                background: backupLoading ? '#CCC' : 'linear-gradient(135deg, #2d7d46 0%, #1e5c34 100%)',
                color: 'white',
                border: 'none',
                borderRadius: '8px',
                padding: '12px 24px',
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button
              ...
              disabled={saving}
              style={{
                background: saving ? '#CCC' : 'linear-gradient(135deg, #FF6B35 0%, #F7931E 100%)',
                color: 'white',
                border: 'none',
                borderRadius: '8px',
                padding: '12px 24px',
                ...
                boxShadow: saving ? 'none' : '0 2px 8px rgba(255, 107, 53, 0.3)',
                transition: 'all 0.2s'
              }}
            >
