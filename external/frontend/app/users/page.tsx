    ...
    try {
      await api.post("/users", {
        ...
        filter_exclude_answering_machine:
          addForm.filter_exclude_answering_machine,
      ...
      setAddError(
        err.response?.data?.detail || "Ошибка при создании пользователя",
      );
    } finally {
                        ...
                        onClick={async () => {
                          try {
                            const res = await api.post(
                              `/users/${editUser.id}/telegram-auth-url`,
                            );
                            if (res.data.url) {
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            const updated = (res.data || []).find(
                              (u: ManagedUser) => u.id === editUser.id,
                            );
                            if (updated) {
                              setEditUser(updated);
                              setEditForm((f) => ({
                  ...
                  {editUser.max_chat_id ? (
                    ...
                    <button
                      ...
                      onClick={async () => {
                        try {
                          const res = await api.post(
                            `/users/${editUser.id}/max-auth-url`,
                          );
                          if (res.data.url) {
                            window.open(res.data.url, "_blank");
