...
export default function CustomDropdown({ label, value, onChange, type }: DropdownProps) {
            ...
            <div className="custom-dropdown" ref={dropdownRef}>
                ...
                {isOpen && (
                    <div className="dropdown-menu">
                        <button type="button" className="dropdown-option" onClick={() => { onChange(''); setIsOpen(false); }}>Все менеджеры</button>
                        {managers.map(m => (
                            <button
                                key={m.id}
                                type="button"
                                className={`dropdown-option ${managerValue === m.id.toString() ? 'is-active' : ''}`}
                                onClick={() => { onChange(m.id.toString()); setIsOpen(false); }}
                            >
                                {m.name}
                            </button>
                        ))}
                ...
                {isOpen && (
                    <div className="dropdown-menu">
                        {valueOptions.map(val => (
                            <label key={val} className="dropdown-option" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <input
                                    type="checkbox"
    ...
    if (type === 'operator') {
        const selectedOperators: string[] = Array.isArray(value) && !isNumberArray(value) ? value as string[] : [];
        ...
        const getDisplayLabel = () => {
            ...
            if (selectedOperators.length === 1) {
                const op = operatorOptions.find(o => o.value === selectedOperators[0]);
                ...
                {isOpen && (
                    <div className="dropdown-menu">
                        {operatorOptions.map(op => (
                            <label key={op.value} className="dropdown-option" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <input
                                    type="checkbox"
