      ...
      if (error.response) {
        ...
        if (status === 400) {
          ...
          if (detail && detail.includes("Authentication")) {
            errorMessage =
              "Ошибка аутентификации с сервисом генерации рекомендаций. Проверьте настройки API ключей";
          } else {
            errorMessage =
              detail || "Ошибка сервера при формировании рекомендаций";
          }
        } else {
          errorMessage =
            detail || `Ошибка ${status}: Не удалось сформировать рекомендации`;
        }
        ...
        if (
          ...
          errorMessage =
            "Не удалось подключиться к серверу. Проверьте подключение к интернету и доступность сервиса.";
        }
      } else {
        errorMessage =
          error.message || "Неизвестная ошибка при формировании рекомендаций";
      }
