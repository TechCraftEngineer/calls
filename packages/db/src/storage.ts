...
export const storage = {
  ...
  getCallByFilename: (filename: string, workspaceId?: number) =>
    callsService.getCallByFilename(filename, workspaceId),
  createCall: (data: any) => callsService.createCall(data),
  getTranscriptByCallId: (callId: number) =>
    callsService.getTranscriptByCallId(callId),
  getEvaluation: (callId: number) => callsService.getEvaluation(callId),
  getCallsWithTranscripts: (params?: any) =>
    callsService.getCallsWithTranscripts(params),
  countCalls: (params?: any) => callsService.countCalls(params),
  addEvaluation: (data: any) => callsService.addEvaluation(data),
  calculateMetrics: (workspaceId?: number) =>
    callsService.calculateMetrics(workspaceId),
  getEvaluationsStats: (params: any) =>
    callsService.getEvaluationsStats(params),
  ...
  verifyWerkzeugHash: (password: string, fullHash: string) =>
    authService.verifyWerkzeugHash(password, fullHash),
  ...
  updateUserFilters: (
    ...
    usersService.updateUserFilters(
      userId,
  ...
  updateUserTelegramSettings: (
    ...
    usersService.updateUserTelegramSettings(
      userId,
      telegramDailyReport,
  ...
  disconnectTelegram: (userId: number) =>
    usersService.disconnectTelegram(userId),
  disconnectMax: (userId: number) => usersService.disconnectMax(userId),

  // Prompt operations
  getPrompt: (key: string, defaultValue?: string) =>
    promptsService.getPrompt(key, defaultValue),
