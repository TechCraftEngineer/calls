...
import { InvitationHelpers, InvitationFactory } from "../../helpers/invitation-helpers";
...
test.describe("Принятие приглашения в рабочее пространство", () => {
  ...
  test.describe("Режим: Регистрация нового пользователя (register-new)", () => {
    test("отображает форму регистрации для нового пользователя по email-приглашению", async ({ page }) => {
    ...
    test("отображает форму регистрации для нового пользователя по ссылке-приглашению", async ({ page }) => {
  ...
  test.describe("Режим: Вход для существующего пользователя (login-existing)", () => {
    test("отображает форму входа для существующего пользователя по email-приглашению", async ({ page }) => {
    ...
    test("отображает форму входа для существующего пользователя по ссылке-приглашению", async ({ page }) => {
      const invitation = InvitationFactory.createLinkInvitation({
        userExists: true,
        requiresPassword: false,
      });
    ...
    test("показывает ссылку на регистрацию с другим email для link-приглашений", async ({ page }) => {
      const invitation = InvitationFactory.createLinkInvitation({
        userExists: true,
        requiresPassword: false,
      });
  ...
  test.describe("Режим: Кнопка присоединения для авторизованного (join-button)", () => {
    test("отображает кнопку присоединения для авторизованного пользователя с паролем", async ({ page }) => {
      const invitation = InvitationFactory.createExistingUserInvitation({
    ...
    test("работает с link-приглашением для любого авторизованного пользователя", async ({ page }) => {
      const invitation = InvitationFactory.createLinkInvitation({
        userExists: true,
      });
  ...
  test.describe("Состояния загрузки и проверки", () => {
    test("показывает состояние загрузки при загрузке приглашения", async ({ page }) => {
      ...
      await page.route("**/api/workspaces.getInvitationByToken", async (route) => {
  ...
  test.describe("Обработка ошибок", () => {
    ...
    test("показывает ошибку при неудачном принятии приглашения", async ({ page }) => {
      ...
      await page.route("**/api/workspaces.acceptInvitation**", async (route) => {
        await route.fulfill({
          status: 400,
          contentType: "application/json",
          body: JSON.stringify({
