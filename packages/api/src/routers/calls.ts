...
async function generateRecommendations(
  ...
  try {
    ...
    if (transcriptText.length > 50000) {
      console.warn(`[recommendations] Transcript too long (${transcriptText.length} chars), truncating`);
...
Сформируй рекомендации в формате JSON-массива строк.`;
    ...
    console.log(`[recommendations] Generating recommendations for call ${callId}`);
    ...
    if (parsed.length === 0) {
      console.warn(`[recommendations] No valid recommendations generated for call ${callId}`);
      return { recommendations: ["Не удалось сформировать рекомендации. Попробуйте позже."] };
    }
    
    console.log(`[recommendations] Generated ${parsed.length} recommendations for call ${callId}`);
    return { recommendations: parsed };
  } catch (error) {
    console.error(`[recommendations] Error generating recommendations for call ${callId}:`, error);
...
function parseRecommendationsJson(text: string): string[] {
  ...
  try {
    ...
    if (Array.isArray(arr)) {
      const validRecommendations = arr.filter((x): x is string => 
    ...
    console.warn('[recommendations] Failed to parse JSON, trying fallback parsing:', error);
  }
