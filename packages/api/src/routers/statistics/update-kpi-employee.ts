...
const updateKpiByEmployeeSchema = z.object({
  ...
  data: z.object({
    kpiBaseSalary: z
      .number({ error: "Базовый оклад должен быть числом" })
      .int({ error: "Базовый оклад должен быть целым числом" })
      .min(0, { message: "Базовый оклад не может быть отрицательным" })
      .max(1_000_000, {
        message: "Базовый оклад не может превышать 1 000 000",
      }),
    kpiTargetBonus: z
      .number({ error: "Целевой бонус должен быть числом" })
      .int({ error: "Целевой бонус должен быть целым числом" })
    ...
    kpiTargetTalkTimeMinutes: z
      .number({ error: "Целевое время разговоров должно быть числом" })
      .int({
        error: "Целевое время разговоров должно быть целым числом",
      })
      .min(0, {
