...
export const getMonthlyKpiGrid = workspaceAdminProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    console.log(
      `[getMonthlyKpiGrid] Всего сотрудников: ${employees.length}, активных с KPI: ${activeEmployees.length}`,
    );

    const totalKpiTarget = activeEmployees.reduce(
      (sum, emp) => sum + (emp.kpiTargetTalkTimeMinutes || 0),
      0,
    );
    console.log(`[getMonthlyKpiGrid] Суммарная цель KPI: ${totalKpiTarget} мин`);
