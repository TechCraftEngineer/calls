...
export const settingsRouter = {
  ...
  updatePrompts: adminProcedure.input(settingsUpdateSchema).handler(async ({ input, context }) => {
    if (input.deepseek_model && input.deepseek_model in DEEPSEEK_MODELS) {
      storage.updatePrompt("deepseek_model", input.deepseek_model, "Selected DeepSeek model");
    }
    if (input.quality_min_value_threshold !== undefined) {
      storage.updatePrompt("quality_min_value_threshold", String(input.quality_min_value_threshold), "Minimum call value for quality evaluation (0-5)");
    }
    if (input.enable_manager_recommendations !== undefined) {
      storage.updatePrompt("enable_manager_recommendations", input.enable_manager_recommendations ? "true" : "false", "Включить генерацию рекомендаций для менеджера (true/false)");
    }
    if (input.telegram_bot_token !== undefined) {
      storage.updatePrompt("telegram_bot_token", input.telegram_bot_token ?? "", "Telegram Bot Token");
    }
    if (input.max_bot_token !== undefined) {
      storage.updatePrompt("max_bot_token", input.max_bot_token ?? "", "MAX Bot Token");
    }
    if (input.prompts) {
      const promptKeys = [
        "summary", "transcribe_incoming", "transcribe_outgoing", "speaker_analysis",
        "speaker_analysis_incoming", "speaker_analysis_outgoing", "value_incoming", "value_outgoing",
        "quality", "manager_recommendations",
  ...
  backup: adminProcedure.handler(async ({ context }) => {
    const dbPath = process.env.BACKEND_DB_PATH ?? resolve(__dirname, "../../../apps/backend/data/db.sqlite");
    ...
    storage.addActivityLog("info", `Резервная копия базы: ${backupFilename} → ${backupPath}`, (context.user as Record<string, unknown>).username as string);
    return { success: true, message: "Резервная копия создана.", path: backupPath };
  }),
};
