...
export const reportsRouter = {
  sendTestTelegram: protectedProcedure.handler(async ({ context }) => {
    const username = (context.user as Record<string, unknown>).username as string;
    ...
    throw new Error("Telegram service not yet integrated - add TelegramService.send_message");
  }),
};
