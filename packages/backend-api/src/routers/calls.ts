...
export const callsRouter = {
  list: protectedProcedure.input(listCallsSchema).handler(async ({ input, context }) => {
    ...
    const callsWithTranscripts = storage.getCallsWithTranscripts({
      ...
      internalNumbers,
      mobileNumbers,
      direction: input.direction === "incoming" || input.direction === "Входящий" ? "Входящий" : input.direction === "outgoing" || input.direction === "Исходящий" ? "Исходящий" : undefined,
  ...
  get: protectedProcedure.input(z.object({ call_id: z.number() })).handler(async ({ input, context }) => {
...
function getInternalNumbersForUser(
  ...
  storage: typeof import("@acme/backend-storage").storage
): string[] | undefined {
  ...
  if (adminUsernames.includes((user.username as string) ?? "")) return undefined;
  return nums.split(",").map((s) => s.trim()).filter(Boolean) || undefined;
}

function getMobileNumbersForUser(
  user: Record<string, unknown>,
  _storage: typeof import("@acme/backend-storage").storage
): string[] | undefined {
