...
function asBool(value: unknown, fallback = true): boolean {
  ...
  if (typeof value === "string") {
    ...
    if (["true", "1", "yes", "active", "enabled"].includes(lowered))
      return true;
    if (["false", "0", "no", "inactive", "disabled"].includes(lowered))
      return false;
  }
  return fallback;
}
...
export function normalizeCall(
  ...
  const directionRaw =
    asString(raw.direction) ?? asString(raw.callType) ?? asString(raw.type);
  const direction =
  ...
  return {
    ...
    employeeExternalId:
      ...
      null,
    numberExternalId:
      asString(raw.numberId) ?? asString(raw.number_id) ?? asString(raw.lineId),
    timestamp,
