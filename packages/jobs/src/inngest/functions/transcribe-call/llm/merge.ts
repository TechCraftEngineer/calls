...
❌ НЕ копируй один и тот же текст в разные сегменты разных спикеров
❌ НЕ дублируй реплики - каждая фраза должна быть только у одного спикера
❌ НЕ создавай перекрывающиеся временные метки (когда end одного > start следующего)
❌ НЕ включай в mergedTranscript дубликаты - только уникальный текст диалога

АЛГОРИТМ РАБОТЫ:
1. АНАЛИЗ СТРУКТУРЫ:
   - Изучи временные метки из диаризированной версии - они показывают границы реплик
   - Определи логику диалога: кто говорит первым, как чередуются спикеры
   - Найди в non-diarized тексте естественные границы реплик (вопросы, ответы, паузы)

2. РАСПРЕДЕЛЕНИЕ ТЕКСТА ПО СПИКЕРАМ:
   - Используй точный текст из non-diarized версии
   - Разбей его на реплики согласно временным меткам из diarized версии
   - Каждый фрагмент текста должен попасть ТОЛЬКО к одному спикеру
   - Если в diarized версии текст неточный - замени его на соответствующий фрагмент из non-diarized

3. ПРОВЕРКА КАЧЕСТВА:
   - Убедись, что каждая фраза из non-diarized текста использована ровно 1 раз
   - Проверь, что временные метки идут по возрастанию без перекрытий
   - Проверь, что спикеры чередуются логично (диалог, а не монолог)

4. СОЗДАНИЕ mergedTranscript:
   - Объедини все сегменты по порядку
   - Каждая реплика должна встречаться только один раз
   - Результат должен читаться как естественный диалог

ПРИМЕР ПРАВИЛЬНОГО РЕЗУЛЬТАТА:
Сегмент 1: SPEAKER_00 (0-2s): "Алло."
Сегмент 2: SPEAKER_01 (2-5s): "Добрый день, меня зовут Никита."
mergedTranscript: "Алло. Добрый день, меня зовут Никита."

ПРИМЕР НЕПРАВИЛЬНОГО РЕЗУЛЬТАТА (НЕ ДЕЛАЙ ТАК):
Сегмент 1: SPEAKER_00 (0-2s): "Алло. Добрый день."
Сегмент 2: SPEAKER_01 (2-5s): "Алло. Добрый день, меня зовут Никита."  ❌ ДУБЛИКАТ "Алло. Добрый день"
mergedTranscript: "Алло. Добрый день. Алло. Добрый день, меня зовут Никита."  ❌ ДУБЛИКАТЫ

ТЕХНИЧЕСКИЕ ТРЕБОВАНИЯ:
- Количество сегментов = количеству в диаризированной версии
- Временные метки (start, end) берутся из диаризированной версии
- Текст (text) берется из non-diarized версии с улучшенной пунктуацией
- Спикеры (speaker) берутся из диаризированной версии
- НЕ выдумывай новых говорящих - используй только SPEAKER_00, SPEAKER_01 и т.д.
...
Верни JSON объект со следующей структурой:
{
  ...
  "mergedTranscript": "Цельный, грамотный русский текст разговора БЕЗ ДУБЛИКАТОВ",
  "quality": {
    "score": 0.85,
...
async function processChunk(
  ...
  const { output } = await generateWithAi({
    ...
    system:
      ...
      "КРИТИЧЕСКИ ВАЖНО: НЕ дублируй текст между сегментами разных спикеров. Каждая фраза должна быть только у одного спикера." +
      (companyContext ? `\n\n${companyContext}` : ""),
    prompt,
    temperature: 0.1,
    maxRetries: 2,
