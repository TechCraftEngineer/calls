...
КЛАССИФИКАЦИЯ:
- "autoanswerer" - только автоответчик/меню (даже если в конце есть короткий сигнал)
- "mixed" - сначала автоответчик, затем живой оператор (перевод на оператора)
- "real_dialogue" - живой разговор между людьми

Признаки автоответчика/голосового меню:
- Приветствие "Вы позвонили в компанию X"
...
ГРАНИЧНЫЕ СЛУЧАИ:
- Если в транскрипте только 1-2 слова ("Алло", "Да") → уверенность "low"
- Если сначала меню, затем разговор → классифицируй как "real_dialogue" с отметкой о начале

Верни JSON с полями:
- isAnsweringMachine: true если это чистый автоответчик, false если живой разговор (включая mixed)
...
export async function isAnsweringMachineWithLlm(
  ...
  try {
    ...
    // Проверяем что output не undefined/null и имеет нужные поля
    if (!output || typeof output !== "object") {
      logger.warn("LLM вернул пустой или невалидный ответ, используем fallback", {
        callId,
        output: output === undefined ? "undefined" : "null",
      });
      return {
        isAnsweringMachine: false,
        confidence: "low",
        reasoning: "LLM вернул пустой ответ, используем безопасный fallback",
      };
    }
    ...
    if (typeof result.isAnsweringMachine !== "boolean" || !result.confidence) {
      logger.warn("LLM вернул неполный ответ, используем fallback", {
        callId,
        hasIsAnsweringMachine: typeof result.isAnsweringMachine === "boolean",
        hasConfidence: !!result.confidence,
      });
      return {
        isAnsweringMachine: false,
        confidence: "low",
        reasoning: "LLM вернул неполный ответ, используем безопасный fallback",
      };
    }
