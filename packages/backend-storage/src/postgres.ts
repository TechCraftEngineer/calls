/**
 * Backend storage - PostgreSQL layer migrated from SQLite
...
export const storage = {
  ...
  async getCallsWithTranscripts(
    ...
    if (dateFrom) {
      conditions.push(eq(schema.calls.timestamp, dateFrom));
    }
    if (dateTo) {
      conditions.push(eq(schema.calls.timestamp, dateTo));
    }
    ...
    if (conditions.length > 0) {
      callsQuery.where(and(...conditions));
    }

    callsQuery.orderBy(desc(schema.calls.timestamp), desc(schema.calls.id));
    callsQuery.limit(limit).offset(offset);

    const results = await callsQuery;
  ...
  async countCalls(
    ...
    const query = db.select({ count: count() }).from(schema.calls);
  ...
  async verifyPassword(username: string, password: string): Promise<boolean> {
    ...
    // Support both bcrypt and legacy pbkdf2 hashes during migration
  ...
  async verifyWerkzeugHash(
    ...
    // Import pbkdf2 only when needed for legacy support
  ...
  async calculateMetrics(): Promise<{
    ...
    const [
      ...
      db
        ...
        .from(schema.calls)
        .where(eq(schema.calls.duration, 0)),
      db
  ...
  > {
    ...
    const conditions = [];
    if (dateFrom) conditions.push(eq(schema.calls.timestamp, dateFrom));
    if (dateTo) conditions.push(eq(schema.calls.timestamp, dateTo));
    if (internalNumbers?.length) {
      conditions.push(schema.calls.internal_number.in(internalNumbers));
    }
